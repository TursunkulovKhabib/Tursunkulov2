...
public interface UserControllerApi {
  ...
  @Operation(summary = "Получение имени пользователя", description = "Возвращает имя пользователя по id")
  @ApiResponses({
      @ApiResponse(responseCode = "200", description = "Имя пользователя успешно получено")
  })
  ResponseEntity<Optional<String>> getUser(@PathVariable int id);
  ...
  @Operation(summary = "Удаление пользователя по username", description = "Удаляет пользователя по username")
  @ApiResponses({
      @ApiResponse(responseCode = "200", description = "Имя пользователя успешно удалено")
  })
  ResponseEntity<Void> deleteByUsername(@PathVariable String username);

  @Operation(summary = "Смена номера телефона пользователя", description = "Обновляет номер телефона пользователя")
  @ApiResponses({
      @ApiResponse(responseCode = "200", description = "Номера телефона успешно обновлен"),
      @ApiResponse(responseCode = "404", description = "Пользователь не найден")
  })
  ResponseEntity<User> patchPhoneNumber(
      @Parameter(description = "Идентификатор пользователя", required = true)
      @PathVariable int id,
      @Parameter(description = "Номер телефона", required = true)
      @PathVariable String phoneNumber);

  @Operation(summary = "Смена электронной почты пользователя", description = "Обновляет электронной почты пользователя")
  @ApiResponses({
      @ApiResponse(responseCode = "200", description = "Email успешно обновлен"),
      @ApiResponse(responseCode = "404", description = "Пользователь не найден")
  })
  ResponseEntity<User> patchEmail(
      @Parameter(description = "Идентификатор пользователя", required = true)
      @PathVariable int id,
  ...
  ResponseEntity<Void> updateUserByUsername(
      @Parameter(description = "Имя пользователя пользователя", required = true)
